...
const inexpensiveFlowers = () => {
    ...
    for (const inexpensive of flowers) {
        if (inexpensive.price < 2.00) {
            filteredFlowers.push(inexpensive)
        }
    }
...
const orangeFlowers = () => {
    const filteredFlowers = []  // Do not modify
    for (const orange of flowers) {
        if (orange.colors.includes("Orange"))
            filteredFlowers.push(orange)
